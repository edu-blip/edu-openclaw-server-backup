...
// Uses high-specificity patterns only — avoids catch-alls that would
// corrupt normal message content (commit hashes, session IDs, etc.)
// ─────────────────────────────────────────────
...
${summary}
  ...
  if (parsed.has_ideas && parsed.ideas?.length > 0) {
    ...
    log(`[USE CASE C] Ideas queued for weekly digest (${parsed.ideas.length} total)`);
  } else {
